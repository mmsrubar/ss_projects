  ...
  //for (int l=0; l < wins_num; l++) {
    ...
    //vstup_komb += param_in;
      ...
      for (int i=0; i < maxidx; i++) {
          ...
          //printf("pixel: %d, chro[%d]: sum(%lu) += %lu\n", 
           //   pixel, i, sum_psnr[i], pow((int)gif_orig[i_index][j_index] - (int)v, 2));
          //printf("%dpx: cmp pixel w %d v %d\n", pixel, gif_orig[i_index][j_index], v);
    ...
    for (int i=0; i < maxidx; i++) {
      ...
      if (sum_psnr[i] == 0) {
        printf("============================\n");
        printf("CHYBA: jakto ze sum vysla 0!\n");
        printf("============================\n");
      }
      //printf("%d: fit:      %f\n",i, 10 * log10(pow(255.0, 2) / (1.0/(255.0*255.0))*sum_psnr[i]));
      fitt_psnr[i] = 10 * log10(pow(255.0, 2) / (1.0/(255.0*255.0))*sum_psnr[i]);
      /*
      printf("fit-part: %lu\n", pow(255.0, 2) / (1.0 / (255.0*255.0)) * sum_psnr[i]);
      printf("chromozom[%d] ma fitness = %lf (sum: %lu)\n", i, fitt_psnr[i], sum_psnr[i]);
      */
    }
        ...
        while (param_generaci++ < PARAM_GENERATIONS) {
            ...
            if (param_generaci % PERIODICLOGG == 0) {
               ...
               for(int j=0; j<param_populace; j++) {
                  printf("{%d, %d}",fitt_psnr[j],uzitobloku((int *)populace[j]));
