...
bool check_ip(unsigned int ip, unsigned int ip_rule, unsigned int mask) {
    ...
    printk(KERN_INFO "compare ip: %pI4l <=> %pI4l\n", &tmp, &ip_rule);
                         ...
                         size_t count, loff_t *off)
{
   ...
   printk(KERN_INFO "src netmask: %pI4\n", &rule_desp->src_netmask);
   ...
   printk(KERN_INFO "dest netmask %pI4\n", &rule_desp->dest_netmask);
                           ...
                           int (*okfn)(struct sk_buff *))
{
  ...
  printk(KERN_INFO "src ip:%pI4, src port:%u; dest ip:%pI4, dest port:%u; proto:%u\n", &src_ip, src_port, &dest_ip, dest_ip_str, dest_port, ip_header->protocol); 
                          ...
                          int (*okfn)(struct sk_buff *))
{
  ...
  printk(KERN_INFO "src ip:%pI4, src port:%u; dest ip:%pI4, dest port:%u; proto:%u\n", &src_ip, src_port, &dest_ip, dest_port, ip_header->protocol); 
   ...
   list_for_each(p, &policy_list.list) {
       ...
       //if a rule doesn't specify as "in", skip it because we have incomming
       //packet here
       if (a_rule->in_out != 1) {
           printk(KERN_INFO "rule %d (a_rule->in_out:%u) not match: in packet, rule doesn't specify as in\n", i, a_rule->in_out);
           continue;  // list_for_each <- take another rule from the list
       } else {
           //check the protocol
           if ((a_rule->proto==1) && (ip_header->protocol != 6)) {
               printk(KERN_INFO "rule %d not match: rule-TCP, packet->not TCP\n", i);
