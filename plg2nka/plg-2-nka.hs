--
-- Projekt:     PLG-2-NKA (FLP)
-- Autor:       Michal Srubar, xsruba03@stud.fit.vutbr.cz
-- Datum:       Thu Apr  9 17:11:27 CEST 2015
--
-- Popis:       
-- Tento program provadi prevod prave linearni (PLG) gramatiky na
-- nedeterministicky konecny automat. PLG muze byt specifikova v souboru nebo
-- zadana na stdin. Dalsi detaily je mozne nalezt v zadani.
...
main = do
  ...
  -- program musi byt spusten s nejakou volbou
  if null args then error (progName++" [volby] [vstup]") else return ()
  ...
  let validInput = (\s n -> if null s then wrongFormat n else s)
  let lineNonTerms  = head $ if null fileContentList then wrongFormat 1 else fileContentList
  let lineTerms     = head $ validInput (drop 1 fileContentList) 2
  let lineStart     = head $ validInput (drop 2 fileContentList) 3
  let lineRules     = validInput (drop 3 fileContentList) 4
...
wrongFormat n
  | n == 1    = error "Vstupni gramatika neobsahuje mnozinu nonterminalnich symbolu."
  | n == 2    = error "Vstupni gramatika neobsahuje mnozinu terminalnich symbolu."
  | n == 3    = error "Vstupni gramatika neobsahuje startovaci symbol."
  | n == 4    = error "Vstupni gramatika neobsahuje mnozinu prepisovacich pravidel."
  | otherwise = error "Funkce wrongFormat zavolana se spatnymi parametry"
...
prgRules g = rt1++rt11++newrt2++rt3++rt4
  where nextIndex = (getLastIndex newrt2)+1
        rt1       = (getPlgRuleT1 g)                            -- A -> aB
        rt11      = (getPlgRuleT11 g)                           -- A -> eps
        newrt2    = (convertPlgRulesType2 1 (getPlgRuleT2 g))   -- A->a1a2..anB
        rt3       = convertPlgRulesType3 nextIndex (getPlgRuleT3 g) -- A->a1a2..an
        rt4       = (getPlgRuleT4 g)                            -- A->B

-- Vysledna prava regularni gramatika (nub odstrani duplicitni nonterminaly a
-- terminaly).
getPrg g = Gramatika nonterms terminals rules startSymbol
  where nonterms    = nub ((map fst $ prgRules g)++nonTerms g)
        terminals   = nub (terms g) -- terminalni symboly bez duplicit
        rules       = removeSimpleRules (prgRules g)
        startSymbol = start g
-- =============================================================================
...
getNka g =
  let nkaDelta    = map (\i -> ((fst i, head$snd i), drop 1 $snd i)) nonEpsRules
      nonEpsRules = (filter (\r -> (length $ snd r) > 1) (rules g))
